...
int main() {
    ...
    for (;;) {
        ...
        connection_info = TCPConnect(sockfd, &servaddr); // TCP handshake
        ...
        if (strcmp(kidsChoice, "2") == 0) { // song request to be replaced
            ...
            sendto(sockfd, (const char *) streamRequest, strlen(streamRequest), 0, 
                (const struct sockaddr *) &servaddr, sizeof(servaddr)); // TODO: replace this
